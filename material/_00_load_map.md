...
각 노트북이 **"무엇을 가르치는가"**에 집중해서 정리한 교육 컨텐츠 로드맵.

## 학습 경로 개요

```
Phase 1 — 기초: LLM과 대화하는 법
  1. API 호출 방식   →  2. 프롬프트 설계   →  3. 멀티턴 대화
...
- 그래서 여기선 InMemory Saver와 SQLite Saver를 비교하는 형태로 문서 작성

**LangGraph에서의 접근**
- `MessagesState`와 `add_messages` reducer: 그래프 상태 자체에 메시지 이력이 내장
- 이후 노트북 11에서 이 패턴을 종합 조립에 직접 사용
...
- 이것이 에이전트 패턴의 기초 → 노트북 11에서 본격 설계
...
### 핵심 메시지
"모든 질문에 '깊은 생각'이 필요하지는 않다. 추론 모델의 작동 원리와 다양한 제어 인자를 이해해야 비용-정확도 최적점을 찾을 수 있다."

### 가르쳐야 할 개념

**추론(Thinking) 모델이란**
- 일반 LLM: 프롬프트 → 바로 답변 생성. 토큰을 순차적으로 출력
- 추론 모델: 프롬프트 → **내부 추론 과정(Chain-of-Thought)** → 답변 생성. "생각한 후 말하기"
- 추론 과정에서 문제를 분해하고, 가설을 세우고, 검증하는 단계를 자동으로 수행
- 핵심 차이: 추론 모델은 **답변 품질을 위해 추가 토큰(시간+비용)을 소비**하는 구조
...
- Gemini 2.5에서 `thinking_budget=0`은 추론 완전 비활성화 (비추론 모델처럼 동작)
- 같은 모델이 budget 설정만으로 추론/비추론 전환 가능 → 별도 모델을 쓸 필요 없음
- **지원 모델 확인이 필수**: `gemini-2.5-flash`, `gemini-2.5-pro`만 thinking 지원. `gemini-2.0-flash`에 thinking_budget을 넣으면 무시되거나 에러

**thinking_budget 심화**
- 값의 의미: 모델이 추론에 사용할 수 있는 **최대 토큰 수**. 실제 사용량은 이보다 적을 수 있음
- `0`: 추론 비활성화. 일반 LLM과 동일하게 동작
- `128 ~ 1024`: 가벼운 추론. 간단한 분류, 짧은 논리 판단
- `1024 ~ 4096`: 중간 수준. 일반적인 분석, 코드 리뷰, 문서 요약 판단
- `4096 ~ 8192`: 깊은 추론. 수학 증명, 복잡한 코드 디버깅, 다단계 논리
- `8192 ~ 24576`: 최대 추론. 연구 수준의 복잡한 문제. 비용 급증 주의
- **주의**: budget을 높인다고 항상 정확도가 올라가는 것은 아님. 단순 문제에 높은 budget은 오히려 "과잉 추론"(overthinking)으로 성능 저하 가능

**thinking token과 과금 구조**
- 추론 모델의 토큰은 3종류로 분리:
  - `input_tokens`: 프롬프트 (사용자 메시지 + system prompt + 대화 이력)
  - `output_tokens`: 최종 답변 텍스트
  - `thinking_tokens`: 내부 추론 과정에 소비된 토큰 (사용자에게 보이지 않음)
- **과금**: thinking_tokens도 output_tokens 단가로 과금됨 → 추론이 깊을수록 비용 증가
- 예시: thinking_budget=4096으로 설정 시, 실제 thinking_tokens가 2000이면 → output 단가 × 2000 추가 비용
- `usage_metadata`에서 각 토큰 종류별 사용량을 확인할 수 있음

**추론 과정 확인: include_thoughts**
- `include_thoughts=True` 설정 시, 모델의 내부 추론 과정을 응답에 포함
- 응답 구조가 달라짐: 단일 텍스트 → content 리스트 (thought 블록 + response 블록)
  ```python
  # include_thoughts=True일 때 응답 content 구조
  [
      {"type": "text", "text": "추론 과정...", "thought": True},  # thinking
      {"type": "text", "text": "최종 답변..."}                     # response
  ]
  ```
- 디버깅, 교육, 품질 검증에 유용: "모델이 왜 이렇게 답했는지" 추론 과정을 직접 확인
- **주의**: include_thoughts는 응답 포맷만 바꿀 뿐, 추론 자체를 활성화/비활성화하지 않음. thinking_budget이 0이면 thought 블록이 비어 있음

**google-genai SDK에서의 설정**
```python
...
response = client.models.generate_content(
    ...
    contents="이 코드의 버그를 찾아줘: ...",
    config={
        "thinking_config": {
            "thinking_budget": 4096,       # 추론 토큰 상한
            "include_thoughts": True        # 추론 과정 포함
        },
        "temperature": 1.0,                # thinking 모델 제약 (아래 참고)
    }
)
```
...
| 모델 | 추론 제어 | 특징 |
|------|----------|------|
| Gemini 2.5 | `thinking_budget` (토큰 수) | 세밀한 토큰 단위 제어 |
| Gemini 3 | `thinking_level` (low/mid/high) | 간편한 단계 제어 |
| OpenAI o1/o3 | `reasoning_effort` (low/medium/high) | 별도 모델 계열 |
| Claude 3.5+ | `extended_thinking` + `budget_tokens` | 토큰 단위 제어, 별도 활성화 플래그 |

- Gemini의 강점: 동일 모델에서 budget만 바꿔 추론/비추론 전환 가능
- OpenAI o1/o3: 추론 전용 모델이므로 비추론과 모델 자체가 다름
- 각 벤더의 접근이 다르므로, **추론 제어의 "개념"을 이해하는 것이 특정 API 사용법보다 중요**
- 이번 노트북에선 구글 생태계만 다루므로 gemini api만 이해하도록 함
...
### 핵심 메시지
"노트북 9에서 도구 하나를 호출하는 법을 배웠다. 이제 여러 노드를 연결하고, 조건에 따라 분기하는 '에이전트 그래프'를 설계할 차례다."

### 가르쳐야 할 개념

**ReAct 패턴이란**
- Reasoning + Acting의 합성어. 모델이 "생각(추론) → 행동(도구 호출) → 관찰(결과 확인) → 다시 생각"을 반복
- 노트북 9의 Tool Calling이 "1회 도구 호출"이었다면, ReAct는 "필요할 때까지 반복하는 루프"
- LangGraph에서는 llm_node ↔ tool_node 사이의 조건부 엣지로 이 루프를 구현
...
**벡터 스토어 비교: Chroma vs FAISS**
- 임베딩 벡터를 저장하고 빠르게 검색하는 특수 DB
- "질의와 가장 유사한 문서 N개를 찾아라" → 이것이 RAG의 Retrieve 단계
- **Chroma**: 영속성 지원(SQLite 백엔드), 메타데이터 필터링, 서버 모드 가능, 설치 간편. 프로토타입~중규모에 적합
- **FAISS**: Meta 개발, 순수 인메모리, 대규모 데이터에서 검색 속도 우수, 영속성은 직접 구현(pickle/파일 저장) 필요
- 비교 포인트: 영속성, 메타데이터 필터링, 검색 속도, 설치 편의성, 프로덕션 확장성
- **실습 설계**: Chroma로 문서 저장+검색 → 같은 데이터를 FAISS로 전환하여 인터페이스와 성능 차이를 직접 비교
- 선택 기준: 프로토타입/소규모 → Chroma / 대규모 프로덕션 → FAISS 또는 Pinecone 등 매니지드 서비스
...
*벡터 스토어: Chroma 메인 + FAISS 비교*
- RAG 파이프라인은 Chroma를 메인 벡터 스토어로 구축
- 동일 데이터를 FAISS로 전환하여 인터페이스/성능 차이 비교 실습 포함 (노트북 12에서 배운 비교를 RAG 맥락에서 확인)
...
*그래프 저장소: NetworkX (실습) + Neo4j (이론 소개)*
- **실습**: NetworkX(순수 Python, 외부 네트워크 불필요)로 그래프 구축/탐색
  1. LLM으로 문서에서 엔티티/관계 추출 (structured output 활용)
  2. NetworkX 그래프에 노드/엣지 추가 + 시각화
  3. 질의 → 관련 엔티티 탐색 → 서브그래프 추출 → context로 주입 → 답변 생성
- **이론 소개**: 프로덕션에서는 Neo4j가 사실상 표준. Neo4j AuraDB(클라우드) + LangChain `Neo4jGraph` 통합으로 확장 가능
- NetworkX는 교육/프로토타입용이며, 영속성과 대규모 그래프 탐색에는 Neo4j가 필요함을 명시
